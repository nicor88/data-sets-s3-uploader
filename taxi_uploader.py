...
def save_to_s3(*, source_url, s3_key, s3_bucket):
    destination_bucket = s3.Bucket(s3_bucket)
    ...
    def _upload_to_s3():
        ...
        destination_obj = destination_bucket.Object(s3_key)
        with requests.get(source_url, stream=True) as response:
            ...
            print(f'{s3_key} saved to S3, total size: {transfer_size_bytes} bytes, {transfer_size_megabytes} MB')

    print(f'File size {source_url} is {file_size_bytes} bytes')
    _upload_to_s3()
...
def get_data_set(data_set_types, start_month, end_month, destination_bucket_name):
    for data_set in data_set_types:
        for year_month in list(generate_months(start_month, end_month)):
            ...
            hive_partition_key = get_hive_partition_key('new_york_taxi', 'fhv_tripdata', year, month, filename)
...
if __name__ == '__main__':
    ...
    start_month_str = vars(args).get('start_month')
    end_month_str = vars(args).get('end_month')
    destination_bucket = vars(args).get('destination_bucket')

    data_sets = ['fhv_tripdata', 'green_tripdata', 'yellow_tripdata']

    get_data_set(data_sets, start_month_str, end_month_str, destination_bucket)
